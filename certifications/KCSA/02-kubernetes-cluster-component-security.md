...
#### Kubernetes Controller Manager and Scheduler recommendations:
...
- The `kubelet` sis the primary `node agen` running on each node
...
- kubelet does not manage containers which which were not created by Kubernetes
...
- run containers with least privilegs to minimize security risks
...
- can enforce `Pod Security Standards` - https://kubernetes.io/docs/concepts/security/pod-security-standards/
